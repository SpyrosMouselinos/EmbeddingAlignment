...
#         if mode not in ['captioning', 'retrieval', 'no_projection']:
...
class TLTLModel(nn.Module):
    def __init__(self, tokenizer, args: FrozenArgs = FrozenArgs()):
        ...
        visual_encoder = args.visual_encoder
        n_visual_tokens = args.n_visual_tokens
        print(f"Using {opt_version} as the LM model.")
    ...
    def get_visual_embs(self,
        ...
        if mode not in ['captioning',
                        'retrieval',
                        'no_projection']:
            raise ValueError(f'mode should be one of'
                             f' ["caption", "retrieval",'
                             f' "no_projection"], got'
                             f' {mode} instead.')
...
def load_tltl(model_dir: str) -> TLTLModel:
    ...
    model = TLTLModel(tokenizer, args)
    model = model.eval()
    model = model.cuda()
