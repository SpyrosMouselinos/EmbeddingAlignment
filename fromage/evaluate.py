            ...
            for i, (image_paths, images, tgt_tokens, token_len) in tqdm.tqdm(enumerate(loader),
                ...
                for model_mode in model_modes:
                    (model_output, full_labels, last_embedding, _, visual_embs) = model(
                        images, tgt_tokens, token_len, mode=model_mode, input_prefix=args.input_prompt,
                        inference=True)  # (N, T, C)
                    ...
                    if model_mode == 'captioning':
                        ...
                        if args.input_prompt is not None:
                            ...
                            prompt_ids = tokenizer(args.input_prompt, add_special_tokens=False,
                        ...
                        if i == 0:
                            ...
                            generated_ids, _, _ = model(input_embs, tgt_tokens, token_len, generate=True,
                                                        num_words=num_words, temperature=0.0, top_p=1.0)
                    ...
                    if i == 0:
                        ...
                        if not args.distributed or (args.rank % ngpus_per_node == 0):
                            ...
                            display_images = torch.cat(
                                [normalized_images.float().cpu(), caption_images, generated_cap_images], axis=2)[
                                             :max_images_to_show]
                            grid = torchvision.utils.make_grid(display_images, nrow=int(max_images_to_show ** 0.5),
                                                               padding=4)
